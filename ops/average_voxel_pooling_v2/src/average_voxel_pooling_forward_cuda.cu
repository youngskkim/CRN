...
__global__ void average_voxel_pooling_forward_kernel(int batch_size, int num_points, int num_channels, int num_voxel_x, int num_voxel_y, int num_voxel_z,
        const int *geom_xyz, const float *input_features, const int *input_pos, float *output_features, float *output_pos, int *pos_memo) {
  ...
  __shared__ int input_pos_shared[THREADS_PER_BLOCK];

  if (thread_sample_idx < total_samples) {
    const int pos = input_pos[thread_sample_idx];
    ...
    input_pos_shared[idx_in_block] = pos;
    ...
    if ((sample_x >= 0 && sample_x < num_voxel_x) && (sample_y >= 0 && sample_y < num_voxel_y) && (sample_z >= 0 && sample_z < num_voxel_z) && (pos != 0)) {
  ...
  for (int i = tidy; i < THREADS_PER_BLOCK && block_sample_idx + i < total_samples; i += THREADS_BLOCK_Y) {
    const int pos = input_pos_shared[i];
    ...
    output_pos[(batch_idx * num_voxel_y * num_voxel_x + sample_y * num_voxel_x + sample_x)] += 1;
  }
}
...
void average_voxel_pooling_forward_kernel_launcher(int batch_size, int num_points, int num_channels, int num_voxel_x, int num_voxel_y, int num_voxel_z,
  ...
  average_voxel_pooling_forward_kernel<<<blocks, threads, 0, stream>>>(batch_size, num_points, num_channels, num_voxel_x, num_voxel_y, num_voxel_z,
                                                                geom_xyz, input_features, input_pos, output_features, output_pos, pos_memo);
