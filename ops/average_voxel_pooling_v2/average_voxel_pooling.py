...
class AverageVoxelPooling(Function):
    ...
    def forward(ctx, geom_xyz: torch.Tensor, input_features: torch.Tensor,
                input_pos: torch.Tensor, voxel_num: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            ...
            input_features (Tensor): feature for each voxel with the
                shape of [B, N, C].
            input_pos (Tensor): indicate positive(valid) grid with the
                shape of [B, N, C].
        ...
        ctx.mark_non_differentiable(input_pos)
        ...
        input_pos = input_pos.reshape(
            (geom_xyz.shape[0], -1, input_pos.shape[-1]))
        ...
        average_voxel_pooling_ext.average_voxel_pooling_forward_wrapper(
            batch_size,
            num_points,
            num_channels,
            voxel_num[0],
        ...
        num_pos = torch.clip(output_num, min=1)  # number of frustum grid assigned to BEV grid
        output_features = output_features / num_pos  # calculate feature average
    ...
    def backward(ctx, grad_output_features, output_num):
        ...
        return None, grad_input_features, None, None


average_voxel_pooling = AverageVoxelPooling.apply
